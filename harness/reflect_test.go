...
func (c *Application) testFunc(a, b int, user models.User) gospf.Result {
	// Line 5
...
func (m Model) Validate(v *gospf.Validation) {
	// Line 26
	v.Required(m.name)
...
func TestGetValidationKeys(t *testing.T) {
	...
	for i, decl := range file.Decls {
		lineKeys := getValidationKeys(fset, decl.(*ast.FuncDecl), map[string]string{"gospf": gospf.REVEL_IMPORT_PATH})
...
func TestProcessBookingSource(t *testing.T) {
	gospf.Init("prod", "github.com/huply/samples/booking", "")
	sourceInfo, err := ProcessSource([]string{gospf.AppPath})
	if err != nil {
		t.Fatal("Failed to process booking source with error:", err)
	}

	CONTROLLER_PKG := "github.com/huply/samples/booking/app/controllers"
...
func BenchmarkProcessBookingSource(b *testing.B) {
	gospf.Init("", "github.com/huply/samples/booking", "")
	gospf.TRACE = log.New(ioutil.Discard, "", 0)
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		_, err := ProcessSource(gospf.CodePaths)
		if err != nil {
